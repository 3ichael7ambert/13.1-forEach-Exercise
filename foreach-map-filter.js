...
function doubleValues(arr){
    arr.forEach(doubling);
    function doubling(item, index, arr) {
      arr[index] = item * 2;
    }
    return arr;
}
...
function showFirstAndLast(arr){
    let newArr = [];
    arr.forEach(firstLast);
    function firstLast(val) {
        let valLength=val.length-1;
        let newVal=val.slice(0,1)+val.slice(valLength);
        newArr.push(newVal); 
    }
    return newArr;
}
...
function addKeyAndValue(arr,key,value){
    // newArr=[];
    arr.forEach(addKeyValue);
    function addKeyValue(val) {     /////function addKeyValue(key,valu)
        val[key]=value;             //////arr.key=value;
    }
    return arr;
}
...
function extractKey(arr, key){          //My errors
    return arr.map(function(value){     //return arr.map(function(value, index){
            return value[key];          ///return arr[index].value;
        });
...
function filterByValue(arr, key) {
    return arr.filter(function(val){
      return val[key] !== undefined; ///return val===key; ////return val.key===true;
    });
}
